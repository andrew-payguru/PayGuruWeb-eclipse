...
public class ClientInvoiceDao {
	...
	public static ClientInvoice loadClientInvoice(Connection connection, int compid, String invno) {
		...
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND invno = ? AND active = ?");
				){
	...
	public static ArrayList<ClientInvoice> loadClientsInvoices(Connection connection, int clientid) {
		...
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE clientid = ? AND active = ?  AND status = ? ORDER BY paydate DESC, invno DESC");
				){
	...
	public static ArrayList<ClientInvoice> loadWalletTransactionsByRefNo(Connection connection, String walletRefNo) {
		ArrayList<ClientInvoice> clientInvoices = new ArrayList<ClientInvoice>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE payref = ? AND active = ?  AND ( status = ? OR status = ? ) ORDER BY paydate DESC, invno DESC");
				){
	...
	public static ArrayList<ClientInvoice> loadClientClientInvoices(Connection connection, int compid, int clientid, boolean period, String from, String to, boolean ignoreStatus) {
		ArrayList<ClientInvoice> clientInvoices = new ArrayList<ClientInvoice>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND clientid = ? AND active = ?" + (ignoreStatus ? "" : " AND status = '"+ClientInvoice.INVOICE_PAID+"' ") + (period?" AND paydate >= '"+from.replace('-', '/')+"' AND paydate <= '"+to.replace('-', '/')+"' " : " ")+"ORDER BY paydate DESC, invno DESC");
				){
			statement.setInt(1, compid);
	...
	public static ArrayList<ClientInvoice> loadClientClientInvoices(Connection connection, int compid, int clientid) {
		return loadClientClientInvoices(connection, compid, clientid, false, DateUtil.getCurrentCCYYMMDDStr(), DateUtil.getCurrentCCYYMMDDStr(),false);
	}

	public static int getInvoiceDayCount(Connection connection, int compid, String date) {
		int count = 0;
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT count(*) AS tot FROM CLIENTINVOICES WHERE compid = ? AND paydate = ? AND status = ? AND canceldate = ''");
				){
			statement.setInt(1, compid);
	...
	public static ArrayList<ClientInvoice> getInactiveSmsInvoices(Connection connection, int compid, int prodid, Date fromdate, Date todate){
		return getInactiveSmsInvoices(connection, compid, prodid, fromdate, todate, null, null);
	}
	
	
	public static ArrayList<ClientInvoice> getInactiveSmsInvoices(Connection connection, int compid, int prodid, Date fromdate, Date todate, String invNo, String regNo){
	...
	public static ClientInvoice searchLatestClientInvoiceByProdRef(Connection connection, int compid, String prodref) {
		ClientInvoice clientInvoice = new ClientInvoice();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND payref ilike ? ORDER BY paydate DESC LIMIT 1");
				){
			statement.setInt(1, compid);
	...
	public static ArrayList<ClientInvoice> getInvoicesByProduct(int compId, int prodId, String dateFrom, String dateTo, Connection connection){
		ArrayList<ClientInvoice> clientInvoices = new ArrayList<ClientInvoice>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND prodid = ? AND paydate >= ? AND paydate <= ?  AND status != ? and active = ? ORDER BY paydate DESC, invno DESC");
				){
	...
	public static ArrayList<ClientInvoice> searchUserProdInvoicesByCell(int compId, int userId, String cellNo, String dateFrom, String dateTo, Connection connection){
		ArrayList<ClientInvoice> clientInvoices = new ArrayList<ClientInvoice>();
		try(
	...
	public static ArrayList<ClientInvoice> searchInvoicesByCell(int compId, String cellNo, String dateFrom, String dateTo, Connection connection){
		ArrayList<ClientInvoice> clientInvoices = new ArrayList<ClientInvoice>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND clientid IN (SELECT clientid FROM CLIENTS WHERE clientcellno LIKE ?) AND paydate >= ? AND paydate <= ? AND status != ? AND active = ?");
				){
			dateFrom = dateFrom.replace("-", "/");
			dateTo = dateTo.replace("-", "/");
			statement.setInt(1, compId);
	...
	public static int getClientInvoiceQuantity(Connection connection, int compid, String dateFrom, String dateTo, int compinternalid) {
		int quantity = 0;
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT count(*) AS quantity FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) WHERE CLIENTINVOICES.compid = ? AND paydate >= ? AND paydate <= ? AND (status = ? OR status = ?)" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid  : ""));
				){
			statement.setInt(1, compid);
	...
	public static double getClientInvoiceTotal(Connection connection, int compid, String dateFrom, String dateTo, int compinternalid) {
		double amt = 0;
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT SUM(payamt) AS total FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) WHERE CLIENTINVOICES.compid = ? AND paydate >= ? AND paydate <= ? AND (status = ? OR status = ?)" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid  : ""));
				){
			statement.setInt(1, compid);
			statement.setString(2, dateFrom);
			statement.setString(3, dateTo);
	...
	public static ArrayList<ClientInvoice> searchInvoices(Connection connection, int compid, String invoiceno){
		...
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES WHERE compid = ? AND invno LIKE ? AND canceldate = '' AND active = ? AND (status = ? OR status = ?) ORDER BY paydate DESC, invno DESC")
		){
			statement.setInt(1, compid);
	...
	public static StringBuilder searchPolicyJSON(Connection connection, int compid, String policyno) {
		...
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT * FROM CLIENTINVOICES JOIN CLIENTINVOICEDATA ON (CLIENTINVOICES.invno = CLIENTINVOICEDATA.invno AND CLIENTINVOICES.compid = CLIENTINVOICEDATA.compid) WHERE CLIENTINVOICES.invno LIKE ? AND CLIENTINVOICES.compid = ? ORDER BY CLIENTINVOICEDATA.invdataref8 DESC, CLIENTINVOICES.invno DESC");
		){
	...
	public static StringBuilder searchPolicysJSON(Connection connection, int compid, int prodid, String fromDate, String toDate, int filterId, String filterWord, boolean usePayDate, int internalCompId, String saleChannel, boolean smsInactive){
		StringBuilder sb = new StringBuilder();
		ArrayList<ClientInvoice> invoices = new ArrayList<ClientInvoice>();
		ArrayList<ClientInvoiceData> invoicedata = new ArrayList<ClientInvoiceData>();
		String sql = "SELECT * FROM CLIENTINVOICES JOIN CLIENTINVOICEDATA ON (CLIENTINVOICES.invno = CLIENTINVOICEDATA.invno AND CLIENTINVOICES.compid = CLIENTINVOICEDATA.compid)" + (smsInactive ? " JOIN CLIENTPAYMENTREFS ON (CLIENTINVOICES.invno = CLIENTPAYMENTREFS.invno) " : " ")
				+ "WHERE CLIENTINVOICES.compid = ? ";
		String dateSql = (usePayDate ? "AND CLIENTINVOICES.paydate >= ? AND CLIENTINVOICES.paydate<= ?" : "AND CLIENTINVOICEDATA.invdataref8 >= ? AND CLIENTINVOICEDATA.invdataref8 <= ?");
		sql+=dateSql;
		String internalCompSql = (internalCompId >0 ? " AND CLIENTINVOICES.prodid IN (SELECT prodid FROM COMPANYPRODUCTS WHERE compid = " + compid + " AND compinternalid = " + internalCompId + ")" : "");
		String payChannelSql = (saleChannel==null||saleChannel.length()<=0||saleChannel.equalsIgnoreCase("ALL") ? "" : " AND CLIENTINVOICES.salechannel = '" + saleChannel + "'") ;
		String smsInactiveSql = " AND CLIENTINVOICES.status = '"+ClientInvoice.INVOICE_INACTIVE+"' AND CLIENTINVOICES.salechannel = 'SMS' AND CLIENTPAYMENTREFS.status = '"+ClientPaymentRef.ACTIVE+"'";
		sql+=internalCompSql;
		sql+=payChannelSql;
		...
		if(filterId==1)
			...
			sql += " AND CLIENTINVOICES.clientid IN (SELECT clientid FROM CLIENTS WHERE clientcellno LIKE '%" + filterWord + "%')";
		else if(filterId==3)
		...
		sql += " AND CLIENTINVOICES.active = ? AND (CLIENTINVOICES.canceldate = ? OR CLIENTINVOICES.canceldate = ?) ORDER BY " + orderBySql + ", CLIENTINVOICEDATA.invno DESC";
		try(
			...
			sb = new StringBuilder("{\"timestamp\" : \""+System.currentTimeMillis()+"\", \"clientinvoices\": " + ClientInvoiceDao.getClientInvoicesJSON(invoices) + ", \"clientinvoicedata\": " + ClientInvoiceDataDao.getClientInvoiceDatasJSON(invoicedata)+"}");
		}catch (Exception e) {
			System.out.println("Error querying CLIENTINVOICES,CLIENTINVOICEDATA: " + e.toString());
		}
		return sb;
	...
	public static ArrayList<String> getDailySaleChannelSaleData(Connection connection, int compid, String from, String to, int compinternalid){
		ArrayList<String> data = new ArrayList<String>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT EXTRACT(DAY FROM TO_DATE(paydate, 'YYYY/MM/DD')) AS day,"
						...
						+ " paydate <= ?" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid + " " : " ") + "AND CLIENTINVOICES.status = ? GROUP BY day, salechannel ORDER BY salechannel,day")
				){
			statement.setInt(1, compid);
	...
	public static ArrayList<String> getMonthlySaleChannelSaleData(Connection connection, int compid, String from, String to, int compinternalid){
		ArrayList<String> data = new ArrayList<String>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT EXTRACT(MONTH FROM TO_DATE(paydate, 'YYYY/MM/DD')) AS month,"
						+ " salechannel, COUNT(*) FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) WHERE CLIENTINVOICES.compid = ? AND paydate >= ? AND"
						+ " paydate<= ?" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid + " " : " ") + "AND CLIENTINVOICES.status = ? GROUP BY month, salechannel ORDER BY salechannel,month")
				){
			statement.setInt(1, compid);
			statement.setString(2, from.replace('-', '/'));
			statement.setString(3, to.replace('-', '/'));
	...
	public static ArrayList<String> getDailyProductSaleData(Connection connection, int compid, String from, String to, int compinternalid){
		ArrayList<String> data = new ArrayList<String>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT EXTRACT(DAY FROM TO_DATE(paydate, 'YYYY/MM/DD')) AS day, "
						+ "COMPANYPRODUCTS.prodname, COUNT(*) FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) "
						+ "WHERE CLIENTINVOICES.compid = ? AND paydate >= ? AND paydate <= ?" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid + " " : " ") + "AND CLIENTINVOICES.status = ? "
	...
	public static ArrayList<String> getMonthlyProductSaleData(Connection connection, int compid, String from, String to, int compinternalid){
		ArrayList<String> data = new ArrayList<String>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT EXTRACT(MONTH FROM TO_DATE(paydate, 'YYYY/MM/DD')) AS month, COMPANYPRODUCTS.prodname,"
						...
						+ " AND paydate >= ? AND paydate <= ?" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid + " " : " ") +  "AND CLIENTINVOICES.status = ?"
	...
	public static int getExpiringPolicyCount(Connection connection, int compid, Date from, Date to, int compinternalid) {
		int count = 0;
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT count(distinct(payref)) FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) WHERE CLIENTINVOICES.compid = ?"
						+ " AND proddate >= ? AND proddate <= ? AND paydate < ?" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid  : "") +  " AND CLIENTINVOICES.status = ?");	
				){
			statement.setInt(1, compid);
	...
	public static int getRenewedPolicyCount(Connection connection, int compid, Date from, Date to,int compinternalid) {
		int count = 0;
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT COUNT(*) FROM CLIENTINVOICES JOIN COMPANYPRODUCTS ON (CLIENTINVOICES.prodid = COMPANYPRODUCTS.prodid) "
						...
						+ "AND paydate <= ? " + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid  : "") + ")" + (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid  : "") + " AND CLIENTINVOICES.status = ?");	
				){
			statement.setInt(1, compid);
	...
	public static ArrayList<String> getPolicyMonthlySalesComparison(Connection connection, int compid, int compinternalid, String from, String to){
		ArrayList<String> data = new ArrayList<String>();
		try(
				PreparedStatement statement = connection.prepareStatement("SELECT EXTRACT(DAY FROM TO_DATE(paydate, 'YYYY/MM/DD')) AS day,"
						...
						+ (compinternalid>0 ? " AND COMPANYPRODUCTS.compinternalid = " + compinternalid + " " : " ") + "AND CLIENTINVOICES.status = ? GROUP BY year,month,day ORDER BY year DESC,month DESC,day");
		){
			statement.setInt(1, compid);
			statement.setString(2, from.replace('-', '/'));
			statement.setString(3, to.replace('-', '/'));
			...
			ResultSet rs = statement.executeQuery();
			while(rs.next()) {
				data.add(rs.getInt("day")+"|"+rs.getInt("year")+"|"+Util.prefixChar(""+rs.getInt("month"),'0',2)+"|"+rs.getInt("count"));
			}rs.close();
		}catch (Exception e) {
			System.out.println("Error querying table CLIENTINVOICES: " + e.toString());
		}
	...
	public static boolean updateInvoice(Connection connection, ClientInvoice clientInvoice) {
		...
		try(
				PreparedStatement statement = connection.prepareStatement("UPDATE CLIENTINVOICES SET clientid = ?, prodid = ?, payamt = ?, payvat = ?"
	...
	public static boolean cancelInvoice(Connection connection, int compid, String invno, int userid, String cancelreason) {
		boolean cancelled = false;
		try(
				PreparedStatement statement = connection.prepareStatement("UPDATE CLIENTINVOICES SET canceldate = ?, canceltime = ?, canceluserid = ?, cancelreason = ?, active = ? WHERE compid = ? AND invno = ?");
				){
